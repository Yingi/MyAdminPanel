...
const getList = (params, resourceName, resourceData) => {
  ...
  if (params.pagination) {
    ...
    // This is what we will use to filter Name
    // We first check if filter object is empty
    // Remember that we need to write the below filter func below
    // based on the resourceName we get.
    // For instance, We can get a nested database and we have to use
    // params.filter.details.Name
    // Note that it becomes more complex if there are multiple Search Input

    if (!isFilterEmpty(params.filter) && resourceName === 'user') {
      let DriverName = values.filter((obj) => {
        console.log(params.filter.details.Name)
        console.log(obj)
        return obj.details.Name === params.filter.details.Name;
      })
      console.log(DriverName)
      values = DriverName;
    }

    if (!isFilterEmpty(params.filter) && resourceName === 'drivers') {
      let DriverName = values.filter((obj) => {
        console.log(params.filter.Name)
        console.log(obj)
        return obj.Name === params.filter.Name;
      })
      console.log(DriverName)
      values = DriverName;
    }
